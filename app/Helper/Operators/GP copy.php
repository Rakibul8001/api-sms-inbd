...
// 	public static function sendSingleMessage($smsSingle, $senderidgatewayInfo)
// 	{
...
// 					$errorCode = $responseData[0];
// 					$errorDescription = $responseData[1];
// 				} else {
// 					$errorCode = 'GPError';
// 					$errorDescription = $responseData[0];
// 				}
// 				SmsGatewayErrors::create([
// 					'sms_id'				=> $smsSingle->id,
		...
		if ($operatorTotalNumbers>0) {
			...
			foreach ($operatorNumbers as $key => $operatorNumber) {
				...
				if ($numberCounter== $batchLimit || $key == $operatorTotalNumbers-1) {
					...
					$phone = "880".$operatorNumber->number;
					// dd($phone);
					// $numArr=["8801768618001","8801789085098"];

					// foreach($operatorTotalNumbers as $key=>$phone){

						try {
	
							  // $response = $client->post($gatewayinfo->operator->multi_url, [
							//     'form_params' => [
							//             "username" 		=> $gatewayinfo->username,
							//             "password" 		=> $gatewayinfo->password,
							//             "apicode"  		=> "5",
